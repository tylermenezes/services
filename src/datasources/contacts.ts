...
let vcards: Record<keyof typeof contacts | "obsidian", VCardContact[]> = {
  apple: [],
  codeday: [],
  obsidian: [],
};
...
export function getContact(
  query: ContactQuery,
  source?: keyof typeof contacts,
): VCardContact | null {
  const sourceCards = source ? vcards[source] : Object.values(vcards).flat();
  ...
  return (
    sourceCards.find((c) => {
      ...
      return (
        c.givenName.toLowerCase() === query.givenName?.toLowerCase() &&
...
async function contactsUpdate() {
  ...
  const cardDavPromise = Promise.all(
    Object.entries(contacts).map(async ([source, client]) => {
      try {
        await client.login();
        const addressBooks = await client.fetchAddressBooks();
        const cards = (
          await client.fetchVCards({ addressBook: addressBooks[0] })
        )
  ...
  const peopleNoteNames = (await obsidian.noteList()).filter((n) =>
    n.startsWith(`people/@`),
  );
  vcards["obsidian"] = (await obsidian.noteReadMulti(peopleNoteNames))
    .map((n): VCardContact | null => {
      let fm = {} as PersonNoteFrontmatter;
      try {
        fm = markdownFrontmatter<PersonNoteFrontmatter>(n.data);
      } catch (ex) {}
      ...
      if (names.length === 0) {
        ...
        givenName = `${names[0]} ${names[1]}`;
        familyName = names.slice(2).join(" ");
      }
      ...
      return {
        ...
        email:
          fm?.email && fm.email.includes(",")
            ? fm.email.split(",").map((e) => e.trim())
            : fm?.email,
        tel:
          fm?.tel && fm.tel.includes(",")
            ? fm.tel.split(",").map((e) => e.trim())
            : fm?.tel,
        org: fm?.org,
        title: fm?.title,
        photo: fm?.photo,
